...
| | |
|---|---|
| Clients | 3 — `Caller`, `Consumer`, `Producer` — `bal discover client ballerinax/kafka` |
| Classes | 6 — `AvroDeserializer`, `AvroSerializer`, `Deserializer`, `Serializer`, `Service`, `Listener` — `bal discover class ballerinax/kafka` |
| Module functions | none |
| Errors | 3 — `Error`, `PayloadBindingError`, `PayloadValidationError` — read one with `bal discover type ballerinax/kafka <Name>` |
...
| Guide | 155 lines — `bal discover guide ballerinax/kafka` |

Guide chunks (5): 1. `Kafka producer`  2. `Kafka consumer`  3. `Listener`  4. `Data serialization`  5. `Concurrency` — `bal discover guide ballerinax/kafka <n>`

## Next

- `bal discover client ballerinax/kafka` — 3 clients, called with `->`
- `bal discover class ballerinax/kafka` — 6 classes and object types, called with `.`
- `bal discover overview ballerinax/kafka -s "<what you need>"` — search every kind at once when you do not know which verb holds it
- `bal discover type ballerinax/kafka <Name> [-r]` — a declaration whole, with the types it names
