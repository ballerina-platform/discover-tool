...
| | |
|---|---|
| Clients | 1 — `Client` — `bal discover client ballerinax/github` |
| Classes | none |
...
| Guide | 111 lines — `bal discover guide ballerinax/github` |

Guide chunks (4): 1. `Step 1: Import the connector`  2. `Step 2: Instantiate a new connector`  3. `Get Private Repositories of Authenticated User`  4. `Create a Private Repository` — `bal discover guide ballerinax/github <n>`

## Next

- `bal discover client ballerinax/github Client` — the client's whole callable surface
- `bal discover overview ballerinax/github -s "<what you need>"` — search every kind at once when you do not know which verb holds it
- `bal discover type ballerinax/github <Name> [-r]` — a declaration whole, with the types it names

## Clients — 1

- `Client` — 903 resource · `bal discover client ballerinax/github Client`
