...
| | |
|---|---|
| Clients | 1 — `Client` — `bal discover client ballerinax/slack` |
| Classes | none |
...
| Guide | 101 lines — `bal discover guide ballerinax/slack` |

Guide chunks (4): 1. `Step 1: Import the module`  2. `Step 2: Instantiate a new connector`  3. `Send a Text Message to General Channel`  4. `Step 4: Run the Ballerina application` — `bal discover guide ballerinax/slack <n>`

## Next

- `bal discover client ballerinax/slack Client` — the client's whole callable surface
- `bal discover overview ballerinax/slack -s "<what you need>"` — search every kind at once when you do not know which verb holds it
- `bal discover type ballerinax/slack <Name> [-r]` — a declaration whole, with the types it names

## Clients — 1

- `Client` — 174 resource · `bal discover client ballerinax/slack Client`
