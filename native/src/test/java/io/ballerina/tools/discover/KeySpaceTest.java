 ...
 *
 * <p>{@code BAL_DISCOVER_UPDATE_KEYSPACE=1 ./gradlew :native:test} rewrites the snapshot after a refresh.
 *
 * @since 0.1.0
 */
    ...
    public void thePayloadKeySpaceIsUnchanged() {
        ...
        Assert.assertNull(difference,
                "Central's payload shape changed. Review the diff, extend CentralDocs if the new field "
                        + "matters, then re-record with BAL_DISCOVER_UPDATE_KEYSPACE=1.\n" + difference);
    }
