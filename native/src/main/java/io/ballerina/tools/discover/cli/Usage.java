...
final class Usage {
     ...
     * against the grammar by {@code DiscoverToolTest}, so a renamed verb fails the build here rather than
     * misleading a reader.
     */
    private static final String SESSION = """
            Typical session:

              0. Don't know which package?   bal discover find "mysql database"
              1. Map + quickstart code:      bal discover overview ballerinax/mysql
              2. Search anywhere with -s (the scope is the verb you attach it to):
                   bal discover client ballerinax/github -s "cache"      # within the client
                   bal discover overview ballerina/http -s "cookie"      # every kind at once
              3. A call and every type it needs:
                   bal discover client ballerinax/github Client delete repos/{owner}/{repo}/actions/caches -r
              4. Setup, auth and worked recipes:  bal discover guide ballerinax/github
            """;
    ...
    private static String examples(String verb) {
        return switch (verb) {
            case "find" -> "  bal discover find kafka messaging\n";
            case "overview" -> """
                      bal discover overview ballerinax/github
                      bal discover overview ballerina/http -s "cookie"
                    """;
            case "client" -> """
                      bal discover client ballerinax/github
                      bal discover client ballerinax/github repos
                      bal discover client ballerinax/github Client delete repos/{owner}/{repo}/actions/caches -r
                      bal discover client ballerinax/twilio -s "message"
                    """;
            case "class" -> """
                      bal discover class ballerina/http
                      bal discover class ballerina/http Cookie
                      bal discover class ballerina/http Cookie toStringValue -r
                    """;
            case "funcs" -> """
                      bal discover funcs ballerina/uuid
                      bal discover funcs ballerina/crypto 'hash*'
                      bal discover funcs ballerina/crypto -s "sha256" -r
                    """;
            case "type" -> """
                      bal discover type ballerina/http ClientRequestError -r
                      bal discover type ballerinax/aws.auth AuthConfig
                      bal discover type ballerinax/github -s "repository"
                    """;
            case "guide" -> """
                      bal discover guide ballerinax/twilio
                      bal discover guide ballerinax/googleapis.sheets 2
                      bal discover guide ballerina/http --module http
                    """;
            case "api" -> "  bal discover api ballerinax/sap\n";
            default -> throw new IllegalArgumentException("no such verb: " + verb);
        };
    }
}
