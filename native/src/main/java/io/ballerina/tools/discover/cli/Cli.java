...
import io.ballerina.tools.discover.views.Find;
 ...
 * recorded payload and a temporary directory. {@link DiscoverTool} is the only place that touches the process.
 *
 * @since 0.1.0
 */
public final class Cli {
        ...
        if ("find".equals(name)) {
            ...
            if (keywords == null || keywords.isEmpty()) {
                return new Failure.Validation(
                        ...
                        "Pass what you are looking for, e.g. bal discover find kafka messaging.");
            }
        }
        if ("type".equals(name)) {
            ...
            if ((names == null || names.isEmpty()) && !searched) {
                return new Failure.Validation(
                        ...
                        "Name it — bal discover type ballerinax/github FullRepository — or search for it: "
    ...
    private static Failure describeParseError(
        ...
        if (cause instanceof CommandLine.UnmatchedArgumentException unmatched) {
            ...
            boolean atRoot = "discover".equals(unmatched.getCommandLine().getCommandSpec().name());
            ...
            return new Failure.Validation(
                    ...
                    "Run `bal discover " + unmatched.getCommandLine().getCommandSpec().name()
    ...
    private static Failure missingArgument(CommandLine.MissingParameterException missing) {
        ...
        if (missingPackage) {
            return new Failure.Validation(
                    ...
                    "Pass 'org/name', e.g. bal discover " + verb + " ballerinax/github.");
        }
    ...
    private static Failure unknownVerb(String token, Commands.Grammar grammar) {
        ...
        String suggestion = token.contains("/")
                ? "A verb comes first: try `bal discover overview " + token + "`. " + verbs
                : verbs;
